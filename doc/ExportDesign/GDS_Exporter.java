...
public class GDS_Exporter
{
	...
	static void Export(Sheet sheet,String name,String type,int[] rows, int[] cols)
	{
		...
		String strType = type;
		if(type.equals("spriteID"))
		{
			strType = "int";
		}
		String _buff_cpp = "const " + strType + " GDS_Exporter::" + name.toLowerCase() +"[" + (row_num*col_num) + "] = \n{\n";
		...
		_buff_h += "\nstatic const " + strType + " " + name.toLowerCase() +"[" + (row_num*col_num) + "];"; 
		_buff_cpp += "\n};";
	...
	public static void main (String[] args)
	{
		try
		{
			...
			//tool
			filename = "GDS_Parameter.h";
			os = new FileOutputStream(filename);
			out = new PrintStream(os);
			
			out.println("#ifndef _DESIGN_PARAM_H_");
			out.println("#define _DESIGN_PARAM_H_");
			out.println("\n");
			
			Sheet[] sheets_design = workbook_1.getSheets();

			int i = 2;
			while (true)
			{
				Cell cell[] =  sheets_design[2].getRow(i);
				String id = MaskString(cell[0].getContents());				
				if (getIntVal(id) == -1)
				{								
					break;					
				}
				...
				{
					String name = MaskString(cell[1].getContents());
					name = name.replace(" ","_");
					name = name.replace("-","_");
					name = name.replace(".","_");
					name = name.toUpperCase();
					String strContent = MaskString(cell[2].getContents());
					strContent = strContent.replace(",","");
					String value = strContent;
					out.println("#define DESIGN_" + name + "\t\t\t(" + value +")" );
				}				
				i++;
			}
	...
	public static void exportDesginTool(Sheet sheet)
	{
