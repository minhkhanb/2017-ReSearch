...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if(GAME()->_notificationEnable)
        {
            ...
            for(int i= 0; i < 3*7; i++)
            {
                ...
                char stringNo[1024];
                sprintf(stringNo, "%s","Time to hunt, come back now!");
                printf("\n test = %s",stringNo);
                notification.soundName = @"sfx_ready.wav";
    ...
    if (MAIN_GAME()->_isProcessingRevive != ShakeDuckOffGame::enumTypeProcessing::REVIVE_NONE)
    {
        MAIN_GAME()->_countTimeDeplay = 1*1000;
    }
...
- (void) InitDisplay
{
       ...
       )
    {
        ...
        if((rect.size.width == 320.000000&& rect.size.height == 568.000000)
           || (rect.size.width == 568.000000&& rect.size.height == 320.000000))
        {
            scale_factor = 2.0f;
        }
