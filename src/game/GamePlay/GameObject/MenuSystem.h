...
#define TYPE_EFFECT_NORMAL					0
#define TYPE_EFFECT_ZOOM_IN_OUT				1
#define TYPE_EFFECT_MOVE_LEFT_RIGHT			2
#define TYPE_EFFECT_MOVE_UP_DOWN			3

class Button
{
public:
	BYTE _typeEffect,_stateEffet;
	float _offX, _offY, _scaleEffectX, _scaleEffectY;
	int _timeDelay;
	//
	...
	void Init(int state, float x, float y, int anchor, int soundID = -1, BYTE typeEffect = TYPE_EFFECT_NORMAL, int timeDlay = 0);
	...
	static int GetFrameCenterX(CSprite* spr, int frameID);
	static int GetFrameCenterY(CSprite* spr, int frameID);
