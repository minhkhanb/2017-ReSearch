...
void CGameStateSetting::Render()
{
	...
	if (MAIN_GAME()->_sub_state == k_LOOP)
	{
		...
		GET_SPRITE(SPRITEID_FONT_SCORE)->DrawAnsiTextFormat(G(), k_SCREEN_WIDTH / 2, k_SCREEN_HEIGHT / 2, _HCENTER, "22");
		/*for (int i = 0; i < _DEC_BUTTON_NUMS; i++)
		{			
...
bool CGameStateSetting::UpdateTouchButton()
{
	return false;
	bool ret = false;
	int index = -1;
...
void CGameStateSetting::UpdateTouch()
{
	for (int i = CGame::_this->_num_touches - 1; i >= 0; i--)
	{
		if (TOUCHS(i)->_flag)
		{
			...
			{
				MAIN_GAME()->_gameStateMainMenu._statePoup = NULL;
				GAME()->ClearAllTouch();
			}
		}
	}
